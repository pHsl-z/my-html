#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# PDF导出功能补丁脚本 - 在项目根目录运行: python3 add_pdf.py
import re
...
# 1. Add PDF export button in the preview dropdown menu (after settings button)
old1 = '                            <button class="btn btn-info dropdown-item" onclick="toggleSettingsPanel()" id="settingsBtn">\n                                <span class="icon">\u2699\ufe0f</span>\n                                \u8bbe\u7f6e\n                            </button>\n                        </div>'
new1 = '                            <button class="btn btn-info dropdown-item" onclick="toggleSettingsPanel()" id="settingsBtn">\n                                <span class="icon">\u2699\ufe0f</span>\n                                \u8bbe\u7f6e\n                            </button>\n                            <button class="btn btn-success dropdown-item" onclick="exportToPdf()" id="pdfBtn">\n                                <span class="icon">\ud83d\udcc4</span>\n                                \u5bfc\u51faPDF\n                            </button>\n                        </div>'
if old1 in content:
    content = content.replace(old1, new1)
    changes += 1
    print("OK: Added PDF button to dropdown menu")
else:
    print("ERROR: dropdown menu pattern not found")

# 2. Add PDF button in share panel "more ways" section
...
if old2 in content:
    ...
    print("ERROR: share panel pattern not found")

# 3. Add exportToPdf function after downloadAsFile function
        ...
        pdf_func = '''
         ...
         function exportToPdf() {
             ...
             try {
                 ...
                 pdfFrame.style.cssText = 'position:fixed;left:-9999px;top:-9999px;width:794px;height:1123px;';
                 ...
                 setTimeout(() => {
                     ...
                     setTimeout(() => { try { document.body.removeChild(pdfFrame); } catch(e) {} }, 5000);
                 }, 500);
             } catch (error) {
                 printPreview();
